<script>
    $(document).ready(function() {
        $("#dataTable").DataTable({
            processing: true,
            ajax: {
                url: "{{ route('coordinaciones.listar') }}", // Ruta de Laravel
                type: "GET",
                headers: {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function(data, type, row) {
                        const url_detalles = "{{ route('coordinaciones.detalles', ':id') }}"
                            .replace(':id', row.id_coordinacion);
                        ...
                        return `
                            <div class="btn-group" role="group">
                                ...
                                <button type="button" class="btn btn-warning btn-sm btn-editar" 
                                        data-id="${row.id_coordinacion}" data-toggle="tooltip" title="Editar">
                                    <i class="fa-duotone fa-solid fa-edit"></i>
                                </button>
                                <button type="button" class="btn btn-${row.estado == 1 ? 'danger' : 'success'} btn-sm btn-cambiar-estado" 
                                        data-id="${row.id_coordinacion}" data-estado="${row.estado}" data-nombre="${row.coordinacion}" 
        ...
        $(document).on('click', '.btn-crear', function() {
            $('#form-crear-o-editar input[name="id_coordinacion"]').val(0);
            $('#form-crear-o-editar input[name="coordinacion"]').val('');

            const titleElement = document.getElementById('modal-formulario-titulo');
            titleElement.innerHTML = '<i class="fa-solid fa-duotone fa-plus"></i> CREAR COORDINACIÓN';
        ...
        $(document).on('click', '.btn-editar', function() {
            ...
            $.get("{{ route('coordinaciones.index') . '/' }}" + id, function(coordinacion) {
                ...
                titleElement.innerHTML =
                    '<i class="fa-solid fa-duotone fa-edit"></i> EDITAR COORDINACIÓN';
                $('#modal-formulario').modal('show');
            });
        });
        ...
        $(document).on('click', '#btn-guardar', function() {
            ...
            const id_coordinacion = $('#form-crear-o-editar input[name="id_coordinacion"]').val();
            const url = id_coordinacion == 0 ?
                "{{ route('coordinaciones.create') }}" // POST -> crear
                :
                "{{ route('coordinaciones.update', ':id') }}"
                .replace(':id', id_coordinacion); // PUT -> actualizar

            const type = id_coordinacion == 0 ? 'POST' : 'PUT';
        ...
        $(document).on('click', '.btn-cambiar-estado', function() {
            ...
            Swal.fire({
                theme: localStorage.getItem('theme') || 'dark',
                title: `¡ATENCIÓN!`,
                html: `¿Estás seguro de <b>${accion}</b> la coordinación <span class="text-primary fw-bold">${nombre}</span>?`,
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('coordinaciones.index') . '/' }}" + id,
                        type: "PATCH",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        ...
                        error: function(xhr) {
                            ...
                            Swal.fire({
                                theme: localStorage.getItem('theme') ||
                                    'dark',
                                title: 'Error',
                                text: `No se pudo ${accion} la coordinación`,
                                icon: 'error'
                            });
                        }
                    });
