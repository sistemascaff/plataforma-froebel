<script>
    $(document).ready(function() {
        ...
        $("#dataTable").DataTable({
            processing: true,
            ajax: {
                url: "{{ route('asignaturas.listar') }}", // Ruta de Laravel
                type: "GET",
                headers: {
        ...
        $(document).on('click', '.btn-crear', function() {
            ...
            $('#form-crear-o-editar input[name="asignatura"]').val('');
            $('#form-crear-o-editar select[name="tipo_calificacion"]').val('');
            $('#form-crear-o-editar select[name="tipo_bloque"]').val('');
        ...
        $(document).on('click', '#btn-guardar', function() {
            ...
            const id_asignatura = $('#form-crear-o-editar input[name="id_asignatura"]').val();
            const url = id_asignatura == 0 ?
                "{{ route('asignaturas.create') }}" // POST -> crear
                :
                "{{ route('asignaturas.update', ':id') }}"
                .replace(':id', id_asignatura); // PUT -> actualizar

            const type = id_asignatura == 0 ? 'POST' : 'PUT';
        ...
        $(document).on('click', '.btn-cambiar-estado', function() {
            ...
            Swal.fire({
                theme: localStorage.getItem('theme') || 'dark',
                title: `¡ATENCIÓN!`,
                html: `¿Estás seguro de <b>${accion}</b> la asignatura <span class="text-primary fw-bold">${nombre}</span>?`,
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('asignaturas.index') . '/' }}" + id,
                        type: "PATCH",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        ...
                        error: function(xhr) {
                            ...
                            Swal.fire({
                                theme: localStorage.getItem('theme') ||
                                    'dark',
                                title: 'Error',
                                text: `No se pudo ${accion} la asignatura`,
                                icon: 'error'
                            });
                        }
                    });
