...
@section('content')
    ...
    @if (date('m-d') === date('m-d', strtotime(Auth::user()->persona?->fecha_nacimiento)))
        <div class="alert alert-warning alert-dismissible fade show shadow-sm text-center border-warning mt-4 mb-4"
            ...
            <p class="mb-0 fs-5">
                Muchas gracias por tu dedicación, gestión y aporte a nuestra institución. ¡Que
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            @include('panel.secretaria_academica.dashboard_botones')
        </div>
    </div>
@endsection

@section('scripts')
    @include('panel.secretaria_academica.dashboard_scripts')
@endsection
