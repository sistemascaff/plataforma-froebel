<script>
    $(document).ready(function() {
        ...
        $("#dataTable").DataTable({
            processing: true,
            ajax: {
                url: "{{ route('gestiones.listar') }}", // Ruta de Laravel
                type: "GET",
                headers: {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        return data.map((periodo, index) =>
                            `<b class="text-info">${index + 1}.</b> ${periodo.periodo}`
                        ).join("<br>");
                    }
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        return data.map((dimension, index) =>
                            `<b class="text-info">${index + 1}.</b> ${dimension.dimension} (${dimension.puntaje_maximo} pts)`
                        ).join("<br>");
                    }
                },
                {
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function(data, type, row) {
                        const url_detalles = "{{ route('gestiones.detalles', ':id') }}"
                            .replace(':id', row.id_gestion);
                        ...
                        return `
                            <div class="btn-group" role="group">
                                ...
                                <button type="button" class="btn btn-warning btn-sm btn-editar" 
                                        data-id="${row.id_gestion}" data-toggle="tooltip" title="Editar">
                                    <i class="fa-duotone fa-solid fa-edit"></i>
                                </button>
                                <button type="button" class="btn btn-${row.estado == 1 ? 'danger' : 'success'} btn-sm btn-cambiar-estado" 
                                        data-id="${row.id_gestion}" data-estado="${row.estado}" data-nombre="${row.anio}" 
        ...
        $(document).on('click', '.btn-crear', function() {
            $('#form-crear-o-editar input[name="id_gestion"]').val(0);
            $('#form-crear-o-editar input[name="anio"]').val(anioActual);

            const titleElement = document.getElementById('modal-formulario-titulo');
            titleElement.innerHTML = '<i class="fa-solid fa-duotone fa-plus"></i> CREAR GESTIÓN';
        ...
        $(document).on('click', '.btn-editar', function() {
            ...
            $.get("{{ route('gestiones.index') . '/' }}" + id, function(gestion) {
                $('#form-crear-o-editar input[name="id_gestion"]').val(gestion.data.id_gestion);
                $('#form-crear-o-editar input[name="anio"]').val(gestion.data.anio);

                const titleElement = document.getElementById('modal-formulario-titulo');
                titleElement.innerHTML =
                    '<i class="fa-solid fa-duotone fa-edit"></i> EDITAR GESTIÓN';
                $('#modal-formulario').modal('show');
            });
        });
        ...
        $(document).on('click', '#btn-guardar', function() {
            ...
            const id_gestion = $('#form-crear-o-editar input[name="id_gestion"]').val();
            const url = id_gestion == 0 ?
                "{{ route('gestiones.create') }}" // POST -> crear
                :
                "{{ route('gestiones.update', ':id') }}"
                .replace(':id', id_gestion); // PUT -> actualizar

            const type = id_gestion == 0 ? 'POST' : 'PUT';
        ...
        $(document).on('click', '.btn-cambiar-estado', function() {
            ...
            Swal.fire({
                theme: localStorage.getItem('theme') || 'dark',
                title: `¡ATENCIÓN!`,
                html: `¿Estás seguro de <b>${accion}</b> la gestión <span class="text-primary fw-bold">${nombre}</span>?`,
                icon: 'question',
