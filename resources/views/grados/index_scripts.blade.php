<script>
    $(document).ready(function() {
        $("#dataTable").DataTable({
            processing: true,
            ajax: {
                url: "{{ route('grados.listar') }}", // Ruta de Laravel
                type: "GET",
                headers: {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        return data.map((curso, index) =>
                            `<b class="text-info">${index + 1}.</b> ${curso.curso}`
                        ).join("<br>");
                    }
                },
                {
                        ...
                        return `
                            <div class="btn-group" role="group" aria-label="Acciones">
                                ...
                                <button type="button" class="btn btn-${row.estado == 1 ? 'danger' : 'success'} btn-sm btn-cambiar-estado" 
                                    data-id="${row.id_grado}" data-estado="${row.estado}" data-nombre="${row.grado}" 
        ...
        $(document).on('click', '.btn-crear', function() {
            $('#form-crear-o-editar input[name="id_grado"]').val(0);
            $('#form-crear-o-editar input[name="grado"]').val('');
            ...
            titleElement.innerHTML = '<i class="fa-solid fa-duotone fa-plus"></i> CREAR GRADO';
        ...
        $(document).on('click', '.btn-editar', function() {
            ...
            $.get("{{ route('grados.index') . '/' }}" + id, function(grado) {
                $('#form-crear-o-editar input[name="id_grado"]').val(grado.data.id_grado);
                $('#form-crear-o-editar input[name="grado"]').val(grado.data.grado);
                $('#form-crear-o-editar input[name="posicion_ordinal"]').val(grado.data
                    .posicion_ordinal);
                $('#form-crear-o-editar select[name="id_nivel"]').val(grado.data
                .id_nivel);

                const titleElement = document.getElementById('modal-formulario-titulo');
                titleElement.innerHTML =
                    '<i class="fa-solid fa-duotone fa-edit"></i> EDITAR GRADO';
                $('#modal-formulario').modal('show');
            });
        });
        ...
        $(document).on('click', '#btn-guardar', function() {
            ...
            const id_grado = $('#form-crear-o-editar input[name="id_grado"]').val();
            const url = id_grado == 0 ?
                "{{ route('grados.create') }}" // POST -> crear
                :
                "{{ route('grados.update', ':id') }}"
                .replace(':id', id_grado); // PUT -> actualizar

            const type = id_grado == 0 ? 'POST' : 'PUT';
        ...
        $(document).on('click', '.btn-cambiar-estado', function() {
            ...
            Swal.fire({
                theme: localStorage.getItem('theme') || 'dark',
                title: `¡ATENCIÓN!`,
                html: `¿Estás seguro de <b>${accion}</b> el grado <span class="text-primary fw-bold">${nombre}</span>?`,
                icon: 'question',
                        ...
                        error: function(xhr) {
                            ...
                            Swal.fire({
                                theme: localStorage.getItem('theme') ||
                                    'dark',
                                title: 'Error',
                                text: `No se pudo ${accion} el grado`,
                                icon: 'error'
                            });
                        }
                    });
