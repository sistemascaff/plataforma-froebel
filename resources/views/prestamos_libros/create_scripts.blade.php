<script>
    $(document).ready(function() {
        ...
        $("#dataTable").DataTable({
            ...
            columns: [{
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function(data, type, row) {
                        if (row.estado == 1) { // Solo mostrar botón si el libro está disponible
                            return `
                            <div class="btn-group" role="group">
                                <button type="button" class="btn btn-success btn-sm btn-agregar" 
                                        data-id="${row.id_libro}" data-codigo="${row.codigo}" data-titulo="${row.titulo}" data-toggle="tooltip" title="Agregar">
        ...
        $(document).on('click', '#btnSave', function() {
            ...
            if (!id_persona) {
                Swal.fire({
                    theme: localStorage.getItem('theme') || 'light',
                    title: "¡No válido!",
                    html: "Selecciona una <b>persona</b> para registrar el préstamo de libros",
                    icon: "info"
                });
                return;
            }
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    registrarPrestamoLibrosAJAX(id_persona, celular, detalles,
                        fecha_devolucion);
                }
            });
        });
        ...
        function registrarPrestamoLibrosAJAX(id_persona, celular, detalles, fecha_devolucion) {
            ...
            $.ajax({
                url: "{{ route('prestamos_libros.create') }}",
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                ...
                success: function(response) {
                    ...
                    window.open("{{ route('prestamos_libros.imprimir', ':id') }}".replace(':id',
                            response.prestamo_libro.id_prestamo_libro), '_blank',
                        'noopener,noreferrer');
                },
                error: function(xhr) {
                    let respuesta = {};
