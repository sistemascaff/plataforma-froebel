<script>
    $(document).ready(function() {
        $("#dataTable").DataTable({
            processing: true,
            ajax: {
                url: "{{ route('areas.listar') }}", // Ruta de Laravel
                type: "GET",
                headers: {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function(data, type, row) {
                        const url_detalles = "{{ route('areas.detalles', ':id') }}"
                            .replace(':id', row.id_area);
                        ...
                        return `
                            <div class="btn-group" role="group">
                                ...
                                <button type="button" class="btn btn-warning btn-sm btn-editar" 
                                        data-id="${row.id_area}" data-toggle="tooltip" title="Editar">
                                    <i class="fa-duotone fa-solid fa-edit"></i>
                                </button>
                                <button type="button" class="btn btn-${row.estado == 1 ? 'danger' : 'success'} btn-sm btn-cambiar-estado" 
                                        data-id="${row.id_area}" data-estado="${row.estado}" data-nombre="${row.area}" 
        ...
        $(document).on('click', '.btn-crear', function() {
            $('#form-crear-o-editar input[name="id_area"]').val(0);
            $('#form-crear-o-editar input[name="area"]').val('');
            ...
            titleElement.innerHTML = '<i class="fa-solid fa-duotone fa-plus"></i> CREAR ÁREA';
        ...
        $(document).on('click', '.btn-editar', function() {
            ...
            $.get("{{ route('areas.index') . '/' }}" + id, function(area) {
                $('#form-crear-o-editar input[name="id_area"]').val(area.data.id_area);
                $('#form-crear-o-editar input[name="area"]').val(area.data.area);
                $('#form-crear-o-editar input[name="abreviatura"]').val(area.data.abreviatura);
                $('#form-crear-o-editar input[name="posicion_ordinal"]').val(area.data.posicion_ordinal);
                $('#form-crear-o-editar select[name="id_campo"]').val(area.data.id_campo);

                const titleElement = document.getElementById('modal-formulario-titulo');
                titleElement.innerHTML =
                    '<i class="fa-solid fa-duotone fa-edit"></i> EDITAR ÁREA';
                $('#modal-formulario').modal('show');
            });
        });
        ...
        $(document).on('click', '#btn-guardar', function() {
            ...
            const id_area = $('#form-crear-o-editar input[name="id_area"]').val();
            const url = id_area == 0 ?
                "{{ route('areas.create') }}" // POST -> crear
                :
                "{{ route('areas.update', ':id') }}"
                .replace(':id', id_area); // PUT -> actualizar

            const type = id_area == 0 ? 'POST' : 'PUT';
        ...
        $(document).on('click', '.btn-cambiar-estado', function() {
            ...
            Swal.fire({
                theme: localStorage.getItem('theme') || 'dark',
                title: `¡ATENCIÓN!`,
                html: `¿Estás seguro de <b>${accion}</b> el area <span class="text-primary fw-bold">${nombre}</span>?`,
                icon: 'question',
                        ...
                        error: function(xhr) {
                            ...
                            Swal.fire({
                                theme: localStorage.getItem('theme') ||
                                    'dark',
                                title: 'Error',
                                text: `No se pudo ${accion} el area`,
                                icon: 'error'
                            });
                        }
                    });
